...
public class QRCodeScannerController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationBarDelegate {
    ...
    func addMaskLayerToVideoPreviewLayerAndAddText(rect: CGRect) {
        ...
        noteText.font = UIFont.init(name: FONT_NAME.FONT_SEMIBOLD, size: 17)
        noteText.string = "Scan QR code inside the box"
        noteText.alignmentMode = kCAAlignmentCenter//CATextLayerAlignmentMode.center
    ...
    private func addButtons(_ view: UIView) {
        ...
        if let cancelImg = cancelImage {
            ...
            cancelButton.frame = CGRect(
                x: 30/*view.frame.width/2 - btnWidthWhenCancelImageNil/2*/,
                y: view.frame.height - 120,
                width: self.view.frame.width - 60/*btnWidthWhenCancelImageNil*/,
                height: height)
            cancelButton.setTitle("Go Back", for: .normal)
        }
        cancelButton.backgroundColor = .systemYellow
        cancelButton.titleLabel?.font = UIFont.init(name: FONT_NAME.FONT_SEMIBOLD, size: 19)
