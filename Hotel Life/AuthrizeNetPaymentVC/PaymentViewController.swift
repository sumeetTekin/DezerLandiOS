...
let kClientName = "665XZa9hXtKG"//"5KP3u95bQpv"
let kClientKey  = "55G78798Ab5tA9FnjK8b2scTKA585N8e9TKR6vetZ55y67NnbbshUP83SQmr4Cxn" //"4d7QC2C38Vub62hZ3SF5XMVzZyMF2nn7zKTXZ5zKUEmXtV2zjyd82mgJ7j96L9TD"//"5FcB6WrfHGS76gHW3v7btBCE3HuuBuke9Pj96Ztfn5R32G5ep42vne7MCWZtAucY"
...
enum PaymentType : String{
    case addCard = "Add Credit Card"
    case payment = "Payment"
}


@objc protocol AuthrizeNetDelegate {
    @objc optional func addCardFunction(_ token : String?)
    @objc optional func paymentDoneFunction(_ token : String?)
}



class PaymentViewController: BaseViewController, UITextFieldDelegate, AlertVCDelegate {
    func alertDismissed() {
        switch type {
        case .addCard:
            self.navigationController?.popViewController(animated: true)
            delegate?.addCardFunction?(generatedToken)
        case .payment:
            self.navigationController?.popViewController(animated: true)
        default:
            self.navigationController?.popViewController(animated: true)
    ...
    var delegate : AuthrizeNetDelegate?
    
    var type = PaymentType.addCard
    
    
    fileprivate var generatedToken: String? = ""
    ...
    override func viewDidLoad() {
        ...
        self.setNavigationBar(title: "Add Your Card")
        navigationController?.navigationBar.barTintColor = UIColor.hexStringToUIColor(hex: "#374654")

    }
    
    override func backToPreviousVCAction(_ sender: UIButton) {
    ...
    func updateTokenButton(_ isEnable: Bool) {
        ...
        if isEnable {
            self.getTokenButton.backgroundColor = UIColor.init(red: 255.0/255.0, green: 212.0/255.0, blue: 41.0/255.0, alpha: 1.0)
        } else {
            self.getTokenButton.backgroundColor = UIColor.init(red: 255.0/255.0, green: 212.0/255.0, blue: 41.0/255.0, alpha: 0.4)
        }
    }
    ...
    func getToken() {
        ...
        handler!.getTokenWithRequest(request, successHandler: { (inResponse:AcceptSDKTokenResponse) -> () in
            DispatchQueue.main.async(execute: {
                ...
                self.generatedToken = inResponse.getOpaqueData().getDataValue()
                ...
                if UserDefaults.standard.value(forKey: "creditValue") != nil{
                    var creditValue = UserDefaults.standard.value(forKey: "creditValue") as? Double
                    let amount = Double(self.amountValue ?? "0")
                    creditValue = (creditValue ?? 0) + (amount ?? 0)
                    UserDefaults.standard.set((creditValue ?? 0.0), forKey: "creditValue")
                }else{
                    var creditValue = 0.0
                    let amount = Double(self.amountValue ?? "0")
                    creditValue = creditValue + (amount ?? 0)
                    UserDefaults.standard.set((creditValue), forKey: "creditValue")
                }
                self.showAlertView()
    ...
    func textChangeDelegate(_ textField: UITextField) {
        NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: "UITextField.textDidChangeNotification"), object: textField, queue: nil, using: { note in
                if (self.validInputs()) {
                    self.updateTokenButton(true)
                } else {
                    self.updateTokenButton(false)
    ...
    private func showAlertView(){
        ...
        objAlert.set(title: "Dezerland", message: "This transaction has been approved", done_title: BUTTON_TITLE.Continue)
